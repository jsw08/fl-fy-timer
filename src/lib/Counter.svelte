<script lang="ts">
  ...
  $: {
    currentClass = classes.find(v => new Date(v.d).getTime() > date.getTime())     
    inSchool = (date.getDay() !== 6 && date.getDay() !== 0) && (currentClass !== undefined) && (date.getHours() >= 6)

    if (inSchool) {
      let distance : number = new Date(currentClass.d).getTime() - date.getTime()
      currentTimes[ "h" ] = Math.floor((distance % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60))
      currentTimes[ "m" ] = Math.floor((distance % (1000 * 60 * 60)) / (1000 * 60))
    ...
    <div class="grid grid-flow-col gap-5 text-center auto-cols-max">
      <div class="flex flex-col">
        <span class="countdown font-mono text-5xl">
          <span style={ `--value:${currentTimes.h};` }></span>
        </span>
        hours 
      </div> 
      <div class="flex flex-col">
        <span class="countdown font-mono text-5xl">
          <span style={ `--value:${currentTimes.m};` }></span>
        </span>
        minutes
      </div> 
      <div class="flex flex-col">
        <span class="countdown font-mono text-5xl">
          <span style={ `--value:${currentTimes.s};` }></span>
        </span>
        seconds
      </div>
    </div>
